...
export function setAuthCookie(
  res: FastifyReply,
  token: string,
  opts?: { isDevelopment?: boolean; maxAgeSeconds?: number },
) {
  const isDevelopment =
    opts?.isDevelopment ?? process.env.NODE_ENV === 'development';
  const maxAge = opts?.maxAgeSeconds ?? 60 * 60 * 24 * 140; // 140 días
  // usar plugin de cookies
  (res as unknown as { setCookie: (...args: any[]) => void }).setCookie(
    'access_token',
    token,
    {
      httpOnly: true,
...
export function clearAuthCookie(res: FastifyReply) {
  (res as unknown as { clearCookie: (...args: any[]) => void }).clearCookie(
    'access_token',
    { path: '/' },
  );
}
